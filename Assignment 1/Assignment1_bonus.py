...
def compute_grads_analytic_svm(data, labels, weight, bias, lmb):
    ...
    grad_weight = grad_weight / data.shape[1] + 2 * lmb * weight_aux
    grad_bias = (grad_weight[:, -1])[:, np.newaxis]
    grad_weight = np.delete(grad_weight, -1, axis=1)
...
def main():
    ...
    for _ in tqdm(range(n_epoch)):
        ...
        for j in range(int(data_train.shape[1] / n_batch)):
            ...
            if SVM_LOSS:
                delta_w, delta_b = compute_grads_analytic_svm(data_train[:, start:end], labels_train[:, start:end],
                                                              weight, bias, lmb)
            else:
